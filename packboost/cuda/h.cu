  ...
  for (int j = 0; j < stride; ++j) {
    ...
    if (base < cols_32M) {
      ...
      if (jj_lane < N) {
        ...
        // --- OPTIMIZATION 1: Skip entirely empty tiles ---
        if (!__any_sync(mask, xfd_local != 0)) {
            continue;
        }
        ...
        for (int k = 0; k < 32; ++k) {
            ...
            // --- OPTIMIZATION 2: Skip iteration if no thread has v=1 ---
            if (!__any_sync(mask, v)) {
                continue;
            }
            ...
            // --- OPTIMIZATION 3: Branchless conditional execution ---
            if (v) {
                const int64_t add = yk;
                ...
                if (tk == 0u) { hf10 += add; hw10 += 1; }
                else          { hf11 += add; hw11 += 1; }

                // d = 2
                tk = lk & 3u; lk >>= 2;
                if      (tk == 0u) { hf20 += add; hw20 += 1; }
                else if (tk == 1u) { hf21 += add; hw21 += 1; }
                else if (tk == 2u) { hf22 += add; hw22 += 1; }
                else               { hf23 += add; hw23 += 1; }
                ...
                for (int d = 3; d < max_depth; ++d) {
                    ...
                    atomicAdd(&sh_high[(idx * 2 + 0) * 32 + lane], static_cast<int>(add));
